...
def compile_ai_summary_data(_comet_api, _opik_api, _opik_client, workspace_name):
    ...
    try:
        ...
        for project in active_projects:
            try:
                # Get traces
                traces = client_api.traces.get_traces_by_project(project_id=project.id, size=100)
                recent_traces = [t for t in traces.content if t.last_updated_at and t.last_updated_at >= three_days_ago]
                ...
                for trace in recent_traces[:10]:  # Analyze up to 10 traces per project
                    ...
                    if trace.error_info:
                        trace_analysis["error_patterns"].append({
                            ...
                            "error": str(trace.error_info)[:200]  # Truncate long errors
                        })
                    
                    # Input/output examples (first few)
                    if len(trace_analysis["input_output_examples"]) < 5 and trace.input and trace.output:
                        trace_analysis["input_output_examples"].append({
                            "trace_name": trace.name,
                            "input_summary": str(trace.input)[:200] + "..." if len(str(trace.input)) > 200 else str(trace.input),
                            "output_summary": str(trace.output)[:200] + "..." if len(str(trace.output)) > 200 else str(trace.output)
                        })
                
                # Get spans for this project
                spans = client_api.spans.get_spans_by_project(project_id=project.id, size=100)
                recent_spans = [s for s in spans.content if s.last_updated_at and s.last_updated_at >= three_days_ago]
                ...
                for span in recent_spans:
                    ...
                    if span.usage and 'total_tokens' in span.usage:
                        ...
                        estimated_cost = span.usage.get('total_tokens', 0) * 0.00001  # Rough estimate
                        trace_analysis["cost_breakdown"]["total_cost"] += estimated_cost
                        if span.model:
                            trace_analysis["cost_breakdown"]["by_model"][span.model] += estimated_cost
        ...
        for dataset in sorted_datasets[:5]:  # Analyze top 5 recent datasets
            dataset_info = {
                ...
                "last_updated": dataset.last_updated_at.isoformat() if dataset.last_updated_at else None,
                ...
                if experiments.content:
                    ...
                    for exp in experiments.content[:3]:  # Top 3 experiments
                        experiment_analysis["recent_experiments"].append({
                            "name": exp.name,
                            "dataset_name": getattr(exp, 'dataset_name', 'Unknown dataset'),
        ...
        ai_summary_data = {
            ...
            "opik_platform_info": {
                ...
                "key_features": ["LLM tracing", "Dataset management", "Experiment tracking", "Prompt versioning", "Model evaluation", "Cost monitoring"]
            }
        }
    ...
    try:
        ...
        for project_name in projects:
            try:
                ...
                latest_experiment_time = max(exp.end_server_timestamp or exp.start_server_timestamp or 0 for exp in experiments)
        ...
        for project_data in most_active_projects:
            ...
            for exp in experiments[:10]:
                ...
                if exp.file_name:
                    file_ext = exp.file_name.split('.')[-1] if '.' in exp.file_name else 'no_ext'
                    experiment_analysis["experiment_patterns"]["file_patterns"][file_ext] += 1
                
                # Duration statistics
                if exp.duration_millis:
                    experiment_analysis["experiment_patterns"]["duration_stats"].append(exp.duration_millis)
                ...
                if len(experiment_analysis["recent_experiments"]) < 5:
                    try:
                        ...
                        if tags:
                            for tag in tags:
                                experiment_analysis["experiment_patterns"]["tag_usage"][tag] += 1
                        
                        # Get parameters
                        params = exp.get_parameters_summary()
                        if params:
                            for param in params:
                                experiment_analysis["experiment_patterns"]["parameter_patterns"][param['name']] += 1
                        
                        # Get metrics
                        metrics = exp.get_metrics_summary()
                        if metrics:
                            for metric in metrics:
                                experiment_analysis["experiment_patterns"]["metric_patterns"][metric['name']] += 1
                        ...
                        if packages:
                            for package in packages:
                                ...
                                if any(framework in pkg_name for framework in ['torch', 'pytorch']):
                                    experiment_analysis["framework_usage"]["PyTorch"] += 1
                                elif any(framework in pkg_name for framework in ['tensorflow', 'tf']):
                                    experiment_analysis["framework_usage"]["TensorFlow"] += 1
                                elif 'keras' in pkg_name:
                                    experiment_analysis["framework_usage"]["Keras"] += 1
                                elif 'sklearn' in pkg_name:
                                    experiment_analysis["framework_usage"]["scikit-learn"] += 1
                                elif 'xgboost' in pkg_name:
                                    experiment_analysis["framework_usage"]["XGBoost"] += 1
                                elif 'lightgbm' in pkg_name:
                                    experiment_analysis["framework_usage"]["LightGBM"] += 1
                                    ...
                                    experiment_analysis["library_usage"]["matplotlib"] += 1
                        ...
                        try:
                            ...
                            if python_version:
                                experiment_analysis["system_info"]["python_versions"][python_version] += 1
                        except:
                            pass
                        ...
                        try:
                            ...
                            if os_type:
                                experiment_analysis["system_info"]["os_types"][os_type] += 1
                        except:
                            pass
                        ...
                        try:
                            ...
                            if hostname:
                                experiment_analysis["system_info"]["hostnames"][hostname] += 1
                        except:
                            pass
                        ...
                        try:
                            ...
                            if assets:
                                for asset in assets:
                                    asset_info["asset_types"][asset.get('type', 'unknown')] += 1
                                    if asset.get('fileSize'):
                                        asset_info["asset_sizes"].append(asset['fileSize'])
                            
                            experiment_info["assets"] = asset_info
                        except:
                            pass
                        
                        experiment_analysis["recent_experiments"].append(experiment_info)
        ...
        common_model_names = ["test-model", "model", "pytorch-model", "tensorflow-model", "production-model"]
        ...
        if experiment_analysis["experiment_patterns"]["duration_stats"]:
            ...
            duration_summary = {"avg_duration_ms": 0, "min_duration_ms": 0, "max_duration_ms": 0, "total_experiments_with_duration": 0}
        ...
        comet_ml_summary_data = {
            ...
            "summary_statistics": {
                ...
                "top_frameworks": dict(experiment_analysis["framework_usage"].most_common(5)),
                "top_libraries": dict(experiment_analysis["library_usage"].most_common(5)),
                "top_tags": dict(experiment_analysis["experiment_patterns"]["tag_usage"].most_common(5)),
                "top_parameters": dict(experiment_analysis["experiment_patterns"]["parameter_patterns"].most_common(5)),
                "top_metrics": dict(experiment_analysis["experiment_patterns"]["metric_patterns"].most_common(5)),
                "file_extensions": dict(experiment_analysis["experiment_patterns"]["file_patterns"].most_common(5)),
                "system_summary": {
                    "python_versions": dict(experiment_analysis["system_info"]["python_versions"]),
                    "os_types": dict(experiment_analysis["system_info"]["os_types"]),
                    "unique_hostnames": len(experiment_analysis["system_info"]["hostnames"])
                }
            },
            "comet_ml_platform_info": {
                ...
                "key_features": ["Experiment tracking", "Model versioning", "Hyperparameter optimization", "Collaborative workspaces", "Model deployment", "Asset management", "System monitoring"]
            }
        }
...
def generate_ai_summary(openai_api_key, _comet_api, _opik_api, _opik_client, workspace_name):
    ...
    try:
        ...
        return {
            ...
            "data_sources": {
                "opik_projects": opik_data.get("project_activity", {}).get("total_projects", 0),
                "opik_traces": opik_data.get("trace_and_span_activity", {}).get("total_traces_3d", 0),
                "comet_ml_projects": comet_ml_data.get("project_activity", {}).get("total_projects", 0),
                "comet_ml_experiments": comet_ml_data.get("experiment_activity", {}).get("total_experiments", 0)
            },
            "model_used": "gpt-4o",
            "generated_at": datetime.now(timezone.utc).isoformat(),
            "token_usage": {
